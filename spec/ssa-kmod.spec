...
if cd ssa; then git pull; else git clone https://github.com/Usable-Security-and-Privacy-Lab/ssa ssa; fi
...
%{?akmod_install}
sudo cp %{_builddir}%{_usrsrc}/ssa/ssa.ko /lib/modules/`uname -r` 
sudo depmod -a
sudo modprobe ssa
...
sudo modprobe -r ssa
rm -f /etc/modules-load.d/ssa.conf
